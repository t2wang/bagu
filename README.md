...
```
InnoDB的行锁是通过给索引上的索引项加锁来实现的，即使在建表的时候没有指定主键，InnoDB会默认创建一个DB_ROW_ID的自增字段为表的主键，并且其主键索引（聚簇索引）为GEN_CLUST_INDEX

主键索引也被称为聚簇索引
```

### SQL语句在用索引的时候，可以同时用多个吗？
